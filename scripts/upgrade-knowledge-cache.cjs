...
// ── Authoritative-source policy (Workstream C — source verification) ──────────
...
//   B) LLM judges authority at call time — rejected here: non-deterministic, un-unit-testable, and
//      the LLM-as-judge layer is a separate (inline, Inc 4) concern; the cache must be reproducible
...
// Pure — classify a source for a stack. No IO. Used by `put` and the `tag` op (and unit tests).
