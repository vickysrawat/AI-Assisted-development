...
argument-hint: "[--deployment | --data | --integrations | --security | --decisions | path]  —  refresh one architecture doc, append a decision, re-run the deployment questionnaire, refresh a subtree (path), or omit to be prompted for the mode (auto-detect recommended; CI auto-detects)"
---
...
If **no argument** was provided, do not default silently — the skill prompts for the mode (Step 0)
with **auto-detect** as the recommended option. The **only** exception is CI / non-interactive runs
(headless or gate-invoked), which skip the prompt and auto-detect changed areas.
