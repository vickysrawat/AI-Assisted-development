---
description: "Remove plugin-managed content from a target project by scope (--full, --skills, --hooks, --rules, --commands, --state). Always dry-runs first and requires explicit CONFIRM before removing anything. memory/, .claude/architecture/, and .claude/graph/ are never removed. Use to clean up after version changes, resolve .hashes conflicts, or fully exit the plugin workflow."
