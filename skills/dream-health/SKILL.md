...
name: dream-health
description: >
  Generate memory/health.html — a self-contained browser dashboard showing confidence
  scores, decay curve, promote candidates, and dream run history with clickable
  justification panels. Invoked by the /dream-health command.
---

# Dream Health Skill — Memory Health Dashboard
...
> **Related specs:** Consent Category C — reads memory files only, never application source (`skills/shared/source-file-consent.md`). Severity vocabulary per `skills/shared/business-context-severity.md`.
...
Read the **canonical memory read set** per `$PLUGIN_DIR/skills/shared/dream-memory-reading-spec.md`
§1 — `CLAUDE.md`, `memory/MEMORY.md`, `memory/topic-*.md`, and `memory/dream-log.md`, in full.
...
Build the **session-id → URL map** using the resolution rules in
`$PLUGIN_DIR/skills/shared/dream-memory-reading-spec.md` §2 (collect ids from MEMORY.md headers,
dream-log operation rows, and topic-file source refs; resolve each via `conversation_search` with
2–3 keywords + date; `unresolved` when no match; cap at the 20 most recent, older → `archive`).
