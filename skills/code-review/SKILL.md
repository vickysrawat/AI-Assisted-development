...
was invoked by another skill/gate (e.g. `checkin` passes `--changed`), skip the menu and use the
cache-aware full scan. Never block a pipeline on a prompt.
...
This is a ledger-writing scanner (findings feed `checkin` Check D), so it defaults to
**repo-only**. When invoked with `--with-deps`, it also reviews the locally-cloned dependency
repos in `additionalDirectories`. Resolve roots via the **bash flavour** of
...
**Every source enumeration in this skill** (Step 0b detection and the review passes) runs once per
`SCAN_ROOT` from `multi_root_scan_roots "$INCLUDE_DEPS"`: announce each non-repo root
(`📁 also scanning dependency: <path>`) and **tag findings with their root**. Note: `checkin`
invokes code-review scoped to the git diff (`--changed`), which never passes `--with-deps`, so the
commit gate stays repo-only by default. Without `--with-deps`, roots = repo only.
