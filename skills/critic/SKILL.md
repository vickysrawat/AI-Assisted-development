...
description: >
  ...
  written to disk. Runs in two modes: ICEA mode critiques an ICEA draft for
  completeness, testability, B1–B7 coverage, and scope-vs-Intent; CODE mode
  critiques generated implementation code for ICEA traceability, simplicity,
  rules compliance, decision transparency, and hidden assumptions. Fires
  automatically inside icea-feature at two gates (after the ICEA draft and
  after code generation but before disk write), and can be invoked directly
  via /critic icea|code for an independent pass. Triggers on: "critique",
  "critic", "second opinion on this spec", "review this before saving",
  "is this over-engineered", "does this match the ICEA".
---

# Critic Skill

_Skill version: 1.0 · Last changed: 2026-06-04 · Plugin compatibility: ≥1.15.0 · Consent: C(internal,icea)|A(code-standalone)_
## Purpose
...
evaluates an artefact **at the moment it is produced and before it is written to
disk**, while the ICEA and the generated code are both still in context.

This is a generator-critic pattern. The generator (the `icea-feature` skill)
produces the artefact; the critic — running as a distinct pass — asks the
questions a careful reviewer would ask before accepting it. Because both the
spec and the output are in context simultaneously, the critic can check
*intent alignment* in a way that a later source-scanning review never can.
...
The critic runs in one of two **modes** (`icea` / `code`) from one of two
**invocation sources** (`internal` / `standalone`). The source determines the
source-file-consent category — see `../shared/source-file-consent.md`.

| Mode | Source | Consent | What it reads |
|---|---|---|---|
| `icea` | `internal` (called by icea-feature after Step 2) | **Category C** | The in-context ICEA draft + architecture docs. Never source. |
| `icea` | `standalone` (`/critic icea ADO-<id>`) | **Category C** | The ICEA file at `docs/icea/ADO-<id>-*.md` + architecture docs. Never source. |
| `code` | `internal` (called by icea-feature inside Step 6) | **Category C** | The in-context generated code + the approved ICEA. Nothing is on disk yet, so no source file is read. |
...
The only path that reads source files from disk is **`code` + `standalone`**,
because in that path the code already exists on disk and there is nothing in
context to critique. All other paths are Category C.
...
`icea-feature` passes the mode and source explicitly. Use them directly. Do not
prompt the developer — the critic runs silently as part of the parent flow.
...
**When called internally**, the critic does **not** print this as a standalone
block. Instead it returns the concerns to `icea-feature`, which folds them into
its existing **Step 3 — Flag Gaps** list so the developer sees one consolidated
set of items before approving. There is **no retry loop in ICEA mode** — the
developer resolves concerns via the normal EDIT/APPROVE cycle.

**When invoked standalone**, print the block above directly.
...
## The REVISE loop (CODE mode only)

When the verdict is `REVISE`, run a bounded regenerate-and-re-critique loop.

```
Generate code (in context)
   │
   ▼
CRITIC ── PASS / PASS WITH NOTES ──► write to disk, return verdict to parent
   │
 REVISE
   │
   ▼
Announce: "🔁 Critic revision 1 of 2 — addressing: {concern summary}"
Regenerate the affected artefacts addressing the listed concerns
   │
   ▼
CRITIC ── PASS / PASS WITH NOTES ──► write to disk
   │
 REVISE
   │
   ▼
...
- **Nothing is written to disk while the verdict is REVISE.** Disk writes happen
  only on `PASS` or `PASS WITH NOTES`, or on an explicit `ACCEPT AS-IS` from the
  developer after surfacing.
...
Choose:
  ✅ ACCEPT AS-IS   — write the latest code despite the notes
  ✏  GUIDE          — tell me specifically what to change, then I retry once
  ❌ HALT           — stop, write nothing
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

On `ACCEPT AS-IS`: write to disk, and record the accepted concerns in the parent
...
- NEVER write generated code to disk while the CODE-mode verdict is `REVISE`.
...
- In ICEA mode there is no retry loop — concerns flow into the existing gap list
  and are resolved through the normal EDIT/APPROVE cycle.
