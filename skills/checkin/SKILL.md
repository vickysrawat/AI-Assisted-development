...
name: checkin
description: >
  Pre-commit quality gate — runs code-review (Check A), ICEA compliance via the
  pr-spec-review engine (Check B), a secrets scan (Check C), business-context
  severity (Check 5b), and the open security/DAST findings gate (Check D) in one
  pass over staged + modified files. Produces a unified pass/fail verdict and a
  pre-filled commit command. Invoked by the /checkin command and at the end of
  icea-implement.
---

# Checkin Skill — Pre-commit quality gate

_Skill version: 1.0 · Last changed: 2026-08-28 · Plugin compatibility: ≥1.14.0 · Consent: A_
...
Runs the pre-commit checks in one pass against your changed files and produces a single
pass/fail verdict. Replaces the manual sequence of code-review + icea-review + secrets
check before every commit. Each check delegates to a shared primitive — this skill
orchestrates and aggregates; it does not re-implement compliance, secrets, or findings logic.
...
Load `$PLUGIN_DIR/skills/code-review/SKILL.md` and apply its **Pass 1 checker categories** to the
...
> **Light path — skip Phase D (Pass 0).** This pre-commit gate runs the LLM checker categories only;
> it does NOT run the code-review skill's deterministic Phase D layer (real builds/eslint are too heavy
> for a per-commit gate). The full deterministic scan + Baseline runs under `/code-review`.
...
Route through the single compliance engine — do not re-implement checks here.
Read `$PLUGIN_DIR/skills/pr-spec-review/SKILL.md` and run it with scope `--icea-only`
and output `--compact` against the staged/unstaged diff, using the ICEA located in Step 2
and the shared context already loaded (do not re-read architecture). Map its compact
verdict to Check B: PASS→✅, WARN→⚠️, FAIL→❌, no ICEA→ℹ️ SKIPPED. The engine supplies
missing-AC and scope-creep detection (via `traceability-mapping-spec.md`).
...
If an ICEA was found in Step 2, the engine's compact verdict already reports missing ACs and
scope creep — fold them into the Check B result. The checks below (manifest delta, D-option
drift) are checkin-specific instrumentation layered on top of that verdict.
...
## Step 5 — Check C: Secrets and sensitive files

Load `$PLUGIN_DIR/skills/shared/secrets-scan-spec.md` and run its canonical functions verbatim
— do not inline secret patterns here. File set = staged + unstaged changes (the spec's
`checkin` invocation):
...
Format with the spec's **checkin — Check C** output block.

Result:
- ✅ **PASS** — `$sens`, `$hits`, and `$data` all empty
- ❌ **FAIL** — any non-empty (HARD gate — always blocks the commit; no skip flag)
...
Emit the **checkin — Check C** output block from
`$PLUGIN_DIR/skills/shared/secrets-scan-spec.md` (sensitive file / secret pattern /
data-in-static-dir, each with its fix). Do not maintain a local copy of that wording.
