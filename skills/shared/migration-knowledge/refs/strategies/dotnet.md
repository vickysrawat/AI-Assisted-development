...
_The concrete, stack-specific commands/paths/examples for a .NET 10 target. The invoking migration
skill's generate/verify phases are stack-agnostic and reference the `{TOKEN}`s below; this file is the
ONLY place the .NET specifics live. See `strategies/README.md` for the token contract._
...
## SKELETON (project structure the orchestrator scaffolds — scaffold phase)
```
{Name}.sln
...
```
- Skeleton verify (scaffold): `dotnet build {Name}.sln --no-incremental 2>&1 | tail -5`
- Cluster work: `--configuration Debug` · release verify: `--configuration Release`
...
# Read ./coverage/report/Summary.txt — compare per-assembly Line coverage to the coverage target.
```
...
## RULES (guardrail rule files deployed to .claude/rules/ at scaffold time)
...
```
Health endpoint: `/healthz`. Dev-run command (dev-run checklist): `dotnet run --project {Name}.Api`.
