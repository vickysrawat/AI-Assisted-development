...
**Committed and PR-reviewed — not gitignored** (v3.0.0). Generated by the `architect` skill (Step 7), wired by `dream-init` Step 7b, refreshed by `/graph-sync`. As the single orientation layer ([ADR 0038](../../docs/adr/0038-knowledge-graph-orientation.md)) the graph is tracked in version control so it is reviewed like source and shared across the team.
