...
name: bug
description: >
  Lightweight bug-fix flow — produces a trimmed Root Cause / Fix / Regression Test spec
  and generates the fix after one approval. Bypasses the full ICEA gate for confirmed bugs
  on existing behaviour. Invoked by the /bug command.
---

# Bug Skill — Lightweight bug fix flow

_Skill version: 1.0 · Last changed: 2026-08-30 · Plugin compatibility: ≥1.14.0 · Consent: B_
...
> **Related specs:** Consent Category B — gate before each source read after the Bug Spec is approved (`skills/shared/source-file-consent.md`). Severity vocabulary per `skills/shared/business-context-severity.md`.
