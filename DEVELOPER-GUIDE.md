...
5. **Rules** (`rules/*.md`) are loaded automatically when Claude edits a file matching the rule's `paths` glob.
   - **Per-project rule scoping (Track A, feature-flagged — `PER_PROJECT_RULES=1` or `dream-init-state.json` `per_project_rules:true`; default OFF):** on a mixed .NET solution the deploy step regenerates the **managed `paths:` block** of the framework overlay rules (`csharp-framework48`/`ef6`/`wcf`/`ado-net-legacy`) to the framework project dirs (scope-by-exception; `csharp-dotnet` and all other rules stay broad/verbatim). Developer-edit protection then compares the rule **body** only (`body:<hash>` in `.hashes`); a machine `paths` rewrite is never mistaken for an edit. Flag OFF = byte-identical to the pre-Track-A deploy. Dry-run: `PER_PROJECT_RULES_DRYRUN=1`.
   - **⚠ Verification spike (non-blocking, before broad enablement):** the runtime honoring of narrow/subfolder `paths` globs and **negation** globs (`!src/Legacy/**`) is a Claude-Code host behavior — confirm (a) a scoped rule reaches context before first-generation (correctness is anyway owned by skills reading `versions[]`, this is reinforcement) and (b) whether negation is supported (if yes, exclude framework dirs from the modern rule for zero overlap; if no, accept the overlap + a precedence note). Until confirmed, keep the flag OFF by default.
