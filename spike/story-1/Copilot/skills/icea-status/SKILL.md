---
name: icea-status
description: Show the current on-disk state of an ADO work item's ICEA / Tech-Spec / tracker files and the single next action. Use when the developer asks for status of an ADO item or types "STATUS ADO-...". Read-only.
---

# Skill: icea-status

_Skill version: 1.0 · Last changed: 2026-07-07 · Consent: C_
_GENERATED FROM `Shared/skills/icea-status/SKILL.md` (L1) — DO NOT EDIT · Copilot-native placement (frontmatter added for discovery) · self-contained (no runtime plugin-dir)_

> **Business context severity:** reports status only — no security findings. The B1–B7 model it does
> not trigger lives in the shared L1 content core — no runtime plugin-dir lookup.
...
"where is this actually blocked?" Lens only; never assume, never attribute in output. The persona-lens
spec lives in the shared L1 content core.
