...
| 3.24.0 | Flag commands prompt when invoked bare (new `flag-prompt-spec.md`); resolves the `scope-flags-spec` vs `interactive-menu-spec` "no flag" contradiction. CI/non-interactive never prompts. Run `/setup-sync` to re-deploy the updated command stubs |
