...
//                      policy/ambiguity handling, merge semantics, read-failure safety, and the
//                      measurable one-read I/O contract.
...
test('read failures preserve existing EXTRACTED edges for affected modules and still remove stale edges for healthy modules', () => {
  ...
  try {
    ...
    const sameModuleHealthy = path.join(repo, 'src', 'web', 'helper.ts');
    const healthyModule = path.join(repo, 'src', 'keep', 'keeper.ts');
    const depFile = path.join(repo, 'src', 'dep', 'index.ts');
    w(failFile, "import '../dep';\n");
    w(sameModuleHealthy, "import '../dep';\n");
    w(healthyModule, "import '../dep';\n");
    w(depFile, 'export const dep = 1;\n');
    ...
    w(graphPath, JSON.stringify({
      ...
      nodes,
      edges: [
        { from: 'web', to: 'gone', type: 'depends', confidence: 'EXTRACTED', reason: 'preserve-on-read-failure' },
    ...
    w(hookPath, [
      ...
      "const fs = require('fs');",
      'const orig = fs.readFileSync;',
      "const failFiles = new Set(JSON.parse(process.env.FAIL_READ_FILES || '[]').map(p => String(p).replace(/\\\\/g, '/')));",
      "const norm = p => String(p).replace(/\\\\/g, '/');",
      'fs.readFileSync = function(file, ...args) {',
      '  const p = norm(file);',
      "  if (failFiles.has(p)) { const e = new Error(p.includes('app.ts') ? 'ENOENT: simulated disappear-after-discovery' : 'EACCES: simulated unreadable file'); e.code = p.includes('app.ts') ? 'ENOENT' : 'EACCES'; throw e; }",
    ...
    const run = runNode(EXTRACT_EDGES, [], repo, {
      NODE_OPTIONS: `--require ${hookPath}`,
      FAIL_READ_FILES: JSON.stringify([fwd(failFile), fwd(sameModuleHealthy)]),
    });
    assert(run.code === 0, `expected exit 0, got ${run.code} (stderr: ${run.stderr})`);
    assert(/2 unreadable file\(s\) across 1 affected module\(s\)/.test(run.stderr), `expected unreadable-file count in warning, got stderr: ${run.stderr}`);
    assert(/web: 2 unreadable file\(s\)/.test(run.stderr), `expected affected module id in warning, got stderr: ${run.stderr}`);

    const graph = JSON.parse(fs.readFileSync(graphPath, 'utf8'));
    const edges = edgeMap(graph);
    assert(edges.get('web\tgone') && edges.get('web\tgone').confidence === 'EXTRACTED', 'EXTRACTED edge from affected module must be preserved');
    assert(!edges.has('keep\tgone'), 'stale EXTRACTED edge from successfully-read module must be removed');
  } finally {
    fs.rmSync(base, { recursive: true, force: true });
  }
});
