...
//                      asserts HIGH-risk drift GATE FAIL/exit 1). ALSO runs the sibling
//                      frontend-parity-replay.cjs against _selftest-fp (target-good => all match;
//                      target-drift => F-19 discovery-layer drift + F-07 assertion drift, both exit 0
//                      because frontend parity is ADVISORY). Prints per-assertion PASS/FAIL.
// What it touches:     READ-ONLY. Spawns node on the verifier + the two replay scripts against
//                      committed fixtures under tests/migration-validation/ and tests/frontend-parity/.
...
const FP = path.join(HERE, '..', 'frontend-parity', 'frontend-parity-replay.cjs');
const FPFIX = path.join(HERE, '..', 'frontend-parity', 'fixtures', '_selftest-fp');
const FP_REC = path.join(FPFIX, 'recordings');
const FP_GOOD = path.join(FPFIX, 'target-good');
const FP_DRIFT = path.join(FPFIX, 'target-drift');
...
// FRONTEND-PARITY mechanics: good target → all match, advisory exit 0
const fpGood = runScript(FP, ['--recordings', FP_REC, '--target', FP_GOOD]);
assert('fp good exits 0 (advisory)', fpGood.status === 0);
assert('fp good all match', /2 match · 0 drift/.test(fpGood.out));

// FRONTEND-PARITY mechanics: drift target → F-19 discovery-only + F-07 assertion, still exit 0 (advisory)
const fpDrift = runScript(FP, ['--recordings', FP_REC, '--target', FP_DRIFT]);
assert('fp drift exits 0 (advisory — never a hard gate)', fpDrift.status === 0);
assert('fp drift F-19 caught by discovery layer only', /F-19 \[HIGH\] drift \[discovery\]/.test(fpDrift.out));
assert('fp drift F-07 caught (assertion layer)', /F-07 \[HIGH\] drift/.test(fpDrift.out));
